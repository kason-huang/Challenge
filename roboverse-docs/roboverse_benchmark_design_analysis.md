...
RoboVerse 是一个**统一的机器人学习平台**，包含三大核心组件：

| 组件 | 职责 | 关键特性 |
|------|------|----------|
| **MetaSim** | 统一仿真器抽象层 | 支持 IsaacSim、SAPIEN、MuJoCo、Genesis 等多后端 |
| **RoboVerse Pack** | 任务定义和场景管理 | 配置驱动的场景和任务系统 |
| **RoboVerse Learn** | 学习算法库 | IL（ACT, Diffusion, DiT, VITA）+ RL（PPO, TD3, SAC） |
...
│                                                         │
│  多仿真器支持           多机器人形态           多任务类型  │
│  (Isaac/SAPIEN)       (Franka/Stretch)      (Pick/Stack)│
│         │                   │                   │        │
│         └───────────────────┼───────────────────┘        │
│                            ▼                            │
│                    MetaSim 统一抽象层                      │
...
class RandomizationCfg:
    ...
    camera: bool = False           # 随机化相机位姿
    light: bool = False            # 随机化光照方向、温度、强度
    ground: bool = False           # 随机化地面
    reflection: bool = False        # 随机化材质（粗糙度、金属度、反射率）
    table: bool = False            # 随机化桌面颜色
    wall: bool = False            # 添加墙壁和天花板，随机化墙壁
    scene: bool = False           # 随机化场景

    level: Literal[0, 1, 2, 3] = 0  # 随机化难度级别
...
| 原则 | 说明 |
|-----|------|
| **自包含** | 每个 episode 包含完整的数据，便于离线评估 |
| **可扩展** | metadata 支持自定义字段 |
| **多模态** | 支持 RGB、Depth、State 等多种观测 |
| **标准化** | 统一的数据格式支持跨任务训练 |
...
## 七、对本项目（VLN评测平台）的参考价值

### 7.1 可直接借鉴的设计

| 设计 | RoboVerse | VLN 项目应用 |
|-----|----------|-------------|
| **L0-L3 随机化** | 分层泛化评测 | 视觉外观随机化（光照、纹理） |
| **SceneRandomizer** | 场景动态切换 | 场景动态切换（建筑物、天气） |
| **HDF5 Episode** | 轨迹数据存储 | VLN 轨迹数据存储 |
| **SPL 指标** | 导航成功率指标 | VLN 导航成功率指标 |
| **Fusion 模型** | 多模态融合 | 多模态（视觉+语言）融合 |
...
objects_layer = SceneLayerCfg(
    ...
    elements=[
        USDAssetPoolCfg(
            name="distractor",
            usd_paths=[
                "assets/furniture/chair1.usd",
                "assets/furniture/table1.usd",
            ],
...
RoboVerse 支持从多个公开数据集迁移任务和轨迹：

| 数据集 | 任务类型 | 主要内容 |
|--------|---------|---------|
| **ManiSkill** | 操作任务 | 工业级机械臂操作 |
| **RLBench** | 操作任务 | 100+ 机器人操作任务 |
| **CALVIN** | 操作任务 | 1000+ 长期任务 |
| **Meta-World** | 操作任务 | 50 个操作任务 |
| **Open6DOR** | 操作任务 | 开门任务 |
| **ARNOLD** | 操作任务 | 多样化操作 |
| **robosuite & MimicGen** | 操作任务 | 模仿学习数据生成 |
| **SimplerEnv** | 操作任务 | 简化操作环境 |
| **GAPartNet** | 操作任务 | 可抓取物体数据集 |
| **GAPartManip** | 操作任务 | 零件操作任务 |
| **GraspNet-1B** | 抓取任务 | 10亿+ 抓取姿态 |
| **GarmentLab** | 操作任务 | 布料操作 |
| **UniDoorManip** | 操作任务 | 统一开门操作 |
| **RLAfford** | 操作任务 | 亲和性学习 |
| **LIBERO** | 操作任务 | 开源操作任务 |

### 8.2 资产转换管道

RoboVerse 提供自动化的资产转换：
...
| 亮点 | 说明 |
|-----|------|
| **统一的抽象层** | MetaSim 屏蔽多模拟器差异 |
| **分层随机化** | L0-L3 渐进式泛化评测 |
| **配置驱动** | 通过配置文件定义场景和随机化 |
| **资产管理** | 自动下载、转换、实例化 |
| **标准化** | 统一的数据格式和评测协议 |
| **大规模数据集** | 支持从 15+ 公开数据集迁移任务 |

### 9.2 对 VLN 评测平台的启示

1. **泛化能力评测**：引入 L0-L3 分层随机化机制，评估模型在不同域的泛化能力
2. **场景管理**：采用配置驱动的场景随机化系统，支持场景动态切换
3. **数据格式**：使用 HDF5 存储 episode 数据，便于离线评估和模仿学习
4. **指标体系**：建立标准化的指标计算和聚合系统
5. **多任务支持**：通过任务注册表支持未来扩展到 ObjectNav、ImageNav 等任务
