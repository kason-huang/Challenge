...
本文档定义了 Challenge 评测系统的数据集格式，支持多种导航任务类型。设计原则：

- **通用性**：单一格式支持 VLN、ObjectNav、ImageNav 等任务
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `trajectory.positions` | [[x,y,z],...] | 每步位置 |
| `trajectory.actions` | [int,...] | 动作序列（0=stop, 1=forward, 2=left, 3=right） |
| `trajectory.observations` | [string,...] | RGB 观测（base64 编码），可选 |
| `metrics.success` | int/float | 是否成功（0/1） |
| `metrics.spl` | float | Success weighted by Path Length |
| `metrics.navigation_error` | float | 最终位置到目标的距离 |
...
benchmark:
  ...
  dataset:
    type: "vln"
    format: "challenge"  # 使用 challenge 格式
...
benchmark:
  ...
  dataset:
    type: "objectnav"
    format: "challenge"
    data_path: "data/datasets/objectnav/hm3d/val.json.gz"
```
